...
@Component({
  ...
  template: `
    ...
    @if (!isSuppressed()) {
    <div class="fixed z-50 flex flex-col items-end gap-3" style="bottom: calc(1.5rem + env(safe-area-inset-bottom)); right: max(1.5rem, env(safe-area-inset-right))">

      <!-- Expanded panel -->
      @if (isOpen()) {
        <div id="contact-panel" class="floating-panel bg-black/90 backdrop-blur-xl border border-white/10 rounded-lg p-4 shadow-2xl w-[calc(100vw-3rem)] max-w-64 animate-panel-in">
          <!-- Header -->
          ...
          <a href="https://wa.me/34601423840?text=Hola%2C%20me%20interesa%20un%20presupuesto%20para%20mi%20web"
            ...
            <span class="w-8 h-8 border border-emerald-500/30 rounded flex items-center justify-center group-hover:border-emerald-500/60 transition-colors">
              <svg aria-hidden="true" class="w-4 h-4 text-emerald-500" viewBox="0 0 24 24" fill="currentColor">
            ...
            <div class="flex flex-col">
              ...
              <span class="text-[10px] text-neutral-500 font-mono">Escríbenos</span>
            </div>
          </a>
          ...
          <a [attr.href]="'tel:' + brand.phone" aria-label="Llamar por teléfono"
            ...
            <span class="w-8 h-8 border border-white/10 rounded flex items-center justify-center group-hover:border-white/30 transition-colors">
              <svg aria-hidden="true" class="w-4 h-4 text-neutral-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex flex-col">
              ...
              <span class="text-[10px] text-neutral-500 font-mono">Llamada directa</span>
            </div>
          </a>
          ...
          <a [attr.href]="'mailto:' + brand.email" aria-label="Enviar correo electrónico"
            ...
            <div class="flex flex-col">
              ...
              <span class="text-[10px] text-neutral-500 font-mono">Cuéntanos tu proyecto</span>
            </div>
          </a>
        </div>
      }
      ...
      <button #contactToggle type="button" (click)="isOpen.set(!isOpen())"
        ...
        [attr.aria-expanded]="isOpen()" [attr.aria-controls]="isOpen() ? 'contact-panel' : null">
        ...
        @if (!isOpen()) {
          <!-- Chat icon -->
          <svg aria-hidden="true" class="w-5 h-5 text-emerald-500 group-hover:text-emerald-400 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <!-- Close icon -->
          <svg aria-hidden="true" class="w-4 h-4 text-neutral-400 group-hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export class FloatingCtaComponent implements AfterViewInit, OnDestroy {
  ...
  @ViewChild('contactToggle') private contactToggle?: ElementRef<HTMLButtonElement>;
  brand = BRAND;
  isOpen = signal(false);
  isSuppressed = signal(false);
  ...
  updateVisibility() {
    ...
    const hidden = overlapsViewport(document.getElementById('contacto')) || overlapsViewport(document.querySelector('footer'));
    this.isSuppressed.set(hidden);
    if (hidden) this.isOpen.set(false);
  }
  ...
  closeOnEscape() {
    ...
    this.contactToggle?.nativeElement.focus();
  }
}
